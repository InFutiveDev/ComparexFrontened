...
import { updateResellerAdmin } from "@/lib/dashboard-api";
...
export function ResellerNotesModal({ row, onClose, onSaved }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const result = await updateResellerAdmin(row.id, { adminNotes: notes });
    ...
    <ModalShell title="Notes" onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <textarea
          ...
          placeholder="Add verification notes, follow-up summary, or next steps"
