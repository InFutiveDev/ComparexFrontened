...
const activityTimeline = [
  { id: 1, title: "Partner inquiry received", detail: "Submitted via reseller application form", time: "3 days ago" },
  { id: 2, title: "Assigned to partnerships", detail: "Routed to partner success team", time: "2 days ago" },
  { id: 3, title: "Discovery call completed", detail: "Partner model and territory discussed", time: "1 day ago" },
  { id: 4, title: "Status updated", detail: "Moved to active partner pipeline", time: "6 hours ago" },
];
...
export function ResellerDetails({ reseller }) {
  return (
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-4">
              <div
                ...
                style={{ backgroundColor: reseller.assigneeColor }}
              >
                {reseller.name
                  .split(" ")
                  .map((part) => part[0])
              ...
              <div>
                ...
                <h1 className="mt-1 text-2xl font-bold text-[#13203F] sm:text-3xl">{reseller.name}</h1>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <PriorityBadge priority={reseller.priority} />
        ...
        <div className="grid gap-4 px-5 py-5 sm:grid-cols-2 sm:px-6 xl:grid-cols-4">
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            ...
            <p className="mt-1 text-2xl font-bold text-[#13203F]">{reseller.score}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            <p className="text-xs font-medium text-slate-500">Source</p>
            <p className="mt-1 text-lg font-bold text-[#13203F]">{reseller.source}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            <p className="text-xs font-medium text-slate-500">Work Type</p>
            <p className="mt-1 text-lg font-bold text-[#13203F]">{reseller.workType}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            <p className="text-xs font-medium text-slate-500">Assignee</p>
            <p className="mt-1 text-lg font-bold text-[#13203F]">{reseller.assignee}</p>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-5 lg:col-span-2">
          <InfoCard title="Contact Information" icon={HiUserCircle}>
            <div className="grid gap-4 sm:grid-cols-2">
              <DetailField label="Full Name">{reseller.name}</DetailField>
              <DetailField label="Company">{reseller.company}</DetailField>
              <DetailField label="Phone Number">
                <a href={`tel:${reseller.phone}`} className="text-[#2D4CC8] hover:underline">
                  {reseller.phone}
                </a>
              </DetailField>
              <DetailField label="Email Address">
                <a href={`mailto:${reseller.email}`} className="text-[#40C3CF] hover:underline">
                  {reseller.email}
                </a>
              </DetailField>
            </div>
          ...
          <InfoCard title="Partner Profile" icon={HiTag}>
            <div className="grid gap-4 sm:grid-cols-2">
              <DetailField label="Category">{reseller.category}</DetailField>
              <DetailField label="Work Type">{reseller.workType}</DetailField>
              <DetailField label="Lead Source">{reseller.source}</DetailField>
        ...
        <div className="space-y-5">
          <InfoCard title="Assignment" icon={HiUserPlus}>
            <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
              <div
                ...
                style={{ backgroundColor: reseller.assigneeColor }}
              >
                {reseller.assigneeInitials}
              </div>
              <div>
                <p className="text-sm font-bold text-[#13203F]">{reseller.assignee}</p>
          ...
          <InfoCard title="Lead Insights" icon={HiChartBar}>
            <div className="space-y-4">
              ...
              <DetailField label="Engagement">
                {reseller.priority === "Hot"
                  ? "High intent — onboard partner within 24 hours"
                  : reseller.priority === "Warm"
                    ? "Moderate intent — schedule partner discovery call"
                    : "Early stage — share partner program details"}
              </DetailField>
            </div>
          </InfoCard>
          ...
          <InfoCard title="Quick Notes" icon={HiSparkles}>
            <p className="text-sm leading-relaxed text-slate-600">
              {reseller.name} from {reseller.company} is exploring reseller partnership in the{" "}
              <span className="font-semibold text-[#13203F]">{reseller.category}</span> segment.
              Inquiry originated from <span className="font-semibold text-[#13203F]">{reseller.source}</span>.
            </p>
          </InfoCard>
        </div>
      </div>
