...
const activityTimeline = [
  { id: 1, title: "Lead created", detail: "Submitted via website form", time: "3 days ago" },
  { id: 2, title: "Assigned to sales", detail: "Routed to account owner", time: "2 days ago" },
  { id: 3, title: "Qualification call", detail: "Business requirements captured", time: "1 day ago" },
  { id: 4, title: "Status updated", detail: "Moved to active pipeline stage", time: "6 hours ago" },
];
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-4">
              <div
                ...
                style={{ backgroundColor: merchant.assigneeColor }}
              >
                {merchant.name
                  .split(" ")
                  .map((part) => part[0])
              ...
              <div>
                ...
                <h1 className="mt-1 text-2xl font-bold text-[#13203F] sm:text-3xl">{merchant.name}</h1>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <PriorityBadge priority={merchant.priority} />
        ...
        <div className="grid gap-4 px-5 py-5 sm:grid-cols-2 sm:px-6 xl:grid-cols-4">
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            ...
            <p className="mt-1 text-2xl font-bold text-[#13203F]">{merchant.score}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            <p className="text-xs font-medium text-slate-500">Source</p>
            <p className="mt-1 text-lg font-bold text-[#13203F]">{merchant.source}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            <p className="text-xs font-medium text-slate-500">Work Type</p>
            <p className="mt-1 text-lg font-bold text-[#13203F]">{merchant.workType}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
            <p className="text-xs font-medium text-slate-500">Assignee</p>
            <p className="mt-1 text-lg font-bold text-[#13203F]">{merchant.assignee}</p>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-5 lg:col-span-2">
          <InfoCard title="Contact Information" icon={HiUserCircle}>
            <div className="grid gap-4 sm:grid-cols-2">
              <DetailField label="Full Name">{merchant.name}</DetailField>
              <DetailField label="Company">{merchant.company}</DetailField>
              <DetailField label="Phone Number">
                <a href={`tel:${merchant.phone}`} className="text-[#2D4CC8] hover:underline">
                  {merchant.phone}
                </a>
              </DetailField>
              <DetailField label="Email Address">
                <a href={`mailto:${merchant.email}`} className="text-[#40C3CF] hover:underline">
                  {merchant.email}
                </a>
              </DetailField>
            </div>
          ...
          <InfoCard title="Business Profile" icon={HiTag}>
            <div className="grid gap-4 sm:grid-cols-2">
              <DetailField label="Category">{merchant.category}</DetailField>
              <DetailField label="Work Type">{merchant.workType}</DetailField>
              <DetailField label="Lead Source">{merchant.source}</DetailField>
        ...
        <div className="space-y-5">
          <InfoCard title="Assignment" icon={HiUserPlus}>
            <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-[#f8fafc] p-4">
              <div
                ...
                style={{ backgroundColor: merchant.assigneeColor }}
              >
                {merchant.assigneeInitials}
              </div>
              <div>
                <p className="text-sm font-bold text-[#13203F]">{merchant.assignee}</p>
            ...
            <div className="space-y-4">
              <div>
                <div className="mb-1 flex items-center justify-between text-xs font-medium text-slate-500">
                  ...
                  <span className="text-[#13203F]">{merchant.score}/100</span>
                </div>
              ...
              <DetailField label="Engagement">
                {merchant.priority === "Hot"
                  ? "High intent — follow up within 24 hours"
                  : merchant.priority === "Warm"
                    ? "Moderate intent — schedule a discovery call"
                    : "Early stage — nurture with product information"}
              </DetailField>
            </div>
          </InfoCard>
          ...
          <InfoCard title="Quick Notes" icon={HiSparkles}>
            <p className="text-sm leading-relaxed text-slate-600">
              {merchant.name} from {merchant.company} is evaluating payment solutions in the{" "}
              <span className="font-semibold text-[#13203F]">{merchant.category}</span> segment.
              Lead originated from <span className="font-semibold text-[#13203F]">{merchant.source}</span>.
            </p>
          </InfoCard>
        </div>
      </div>
